...
const UsageStatsModal = ({ isOpen, onClose, baths = [] }) => {
  ...
  const totalRunning = safeBaths.reduce((acc, bath) => acc + ((bath.circuits || []).filter(c => { 
      const s = c.status ? c.status.toLowerCase().trim() : ''; 
      return s === 'running' && c.progress < 100; 
  }).length), 0);

  const totalFree = safeBaths.reduce((acc, bath) => acc + ((bath.circuits || []).filter(c => { 
      const s = c.status ? c.status.toLowerCase().trim() : 'free'; 
      return (s === 'free' || s === 'finished' || c.progress >= 100) && s !== 'maintenance'; 
  }).length), 0);

  const totalMaint = safeBaths.reduce((acc, bath) => acc + ((bath.circuits || []).filter(c => 
      c.status === 'maintenance'
  ).length), 0);

  const totalCircuits = totalRunning + totalFree + totalMaint;

  // 2. Dados para o Gráfico
  const data = [
    { name: 'Em Uso', value: totalRunning, color: '#d97706', icon: Activity, bg: 'bg-amber-100', text: 'text-amber-700' }, 
    { name: 'Livres', value: totalFree, color: '#059669', icon: CheckCircle2, bg: 'bg-emerald-100', text: 'text-emerald-700' },    
    { name: 'Manutenção', value: totalMaint, color: '#be123c', icon: AlertOctagon, bg: 'bg-rose-100', text: 'text-rose-700' } 
  ];

  const chartData = data.filter(item => item.value > 0);

  // 3. Renderiza o Texto Central (Com verificação de segurança)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden border border-slate-200 flex flex-col md:flex-row max-h-[90vh]">
